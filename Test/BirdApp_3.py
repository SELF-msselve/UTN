...
class Unifica_Screen(Screen):
    ...
    def genera_listas(self, header, table, lista):
        ...
        for column_name in lista:
            self.ids[header].add_widget(MDLabel(text=str(column_name), bold=True, size_hint_y=None, height=40))

        for index, row in df_global[lista].iterrows():
            for cell in row:
                if index % 2 == 0:
                    self.ids[table].add_widget(MDLabel(text=str(cell), size_hint_y=None, height=40, theme_text_color="Custom", text_color=(0, 1, 1, 1)))
                else:
                    self.ids[table].add_widget(MDLabel(text=str(cell), size_hint_y=None, height=40, theme_text_color="Custom", text_color=(1, 1, 1, 1)))


class Background(Widget):
    # Objetos para crear nubes y el piso.
...
# Clase que crea el pájaro con sus imágenes de movimiento
class Bird(Image):
...
class GameWindow(Unifica_Screen):
    ...
    def check_collision(self):
        bird = self.ids.bird
        # Verifica las coliciones del pájaro
        is_colliding = False
    ...
    def game_over(self):
        ...
        self.text_input = MDTextField(hint_text=self.entered_text)
        self.secondary_layout.add_widget(self.text_input)

        # Crear un botón para cerrar la ventana secundaria
        close_button = MDRaisedButton(text="Cerrar Ventana Secundaria")
    ...
    # Función que se llama al cerrar el popup y actualiza el df de Score.
...
kv = Builder.load_file('BirdApp_3.kv')

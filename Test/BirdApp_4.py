...
db = SqliteDatabase('./Test/scores.db')
...
# Se crea y lee la tabla de Score
db.connect()
...
class Unifica_Screen(Screen):
    ...
    def genera_listas(self):
        self.ids['header_layout'].clear_widgets()
        self.ids['table_layout'].clear_widgets()
        
        self.ids['header_layout'].cols = 2
        self.ids['table_layout'].cols = 2
        # Nombre de las columnas.
        self.ids['header_layout'].add_widget(Label(text='Nombre', bold=True, size_hint_y=None, height=40))
        self.ids['header_layout'].add_widget(Label(text='Score', bold=True, size_hint_y=None, height=40))

        for scores in Score.select().order_by(Score.score.desc()):
            self.ids['table_layout'].add_widget(Label(text=scores.name, size_hint_y=None, height=40, color=(0,1,1,1)))
            self.ids['table_layout'].add_widget(Label(text=str(scores.score), size_hint_y=None, height=40, color=(0,1,1,1)))
...
class ScoreWindow(Unifica_Screen):
    def on_enter(self):
        Unifica_Screen.genera_listas(self)
...
#Levanta un popup para que el jugador cargue su nombre y registre su score.  
    ...
    def close_popup(self, instance):
        ...
        score_to_db = Score(name=entered_text, score=int(self.ids.score.text))
        score_to_db.save()
        # global df_global
...
kv = Builder.load_file('BirdApp_4.kv')
